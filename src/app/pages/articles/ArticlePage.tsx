  ...
  if (!article) return (
    <div className="min-h-screen bg-ink-950 text-white flex items-center justify-center" dir="rtl">
      <div className="text-center">
        ...
        <Link to="/articles" className="text-royal-400 hover:underline">← العودة للمقالات</Link>
      </div>
    </div>
  );
  ...
  return (
    <div className="bg-ink-950 text-white min-h-screen" dir="rtl">
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 pt-8">
        <Link to="/articles" className="flex items-center gap-2 text-gray-400 hover:text-royal-300 transition-colors text-sm">
          <ArrowRight size={16} />
          العودة للمقالات
        </Link>
      </div>
      ...
      <article className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        {article.tags?.length > 0 && (
          <div className="flex gap-2 flex-wrap mb-4">
            {article.tags.map((tag, i) => (
              <span key={i} className="text-sm text-royal-300 bg-royal-900/30 px-3 py-1 rounded-full">{tag}</span>
            ))}
          </div>
        )}
        ...
        {/* التاريخ والتصنيف */}
        <div className="flex items-center gap-4 text-gray-500 text-sm mb-10 border-b border-ink-700 pb-6">
          {article.category && (
            <span className="bg-royal-600/20 text-royal-400 px-3 py-1 rounded-full text-xs font-semibold">
              {article.category}
            </span>
          )}
        ...
        {/* ===== النص الرئيسي ===== */}
        <style>{`
          .article-body { font-size:1.15rem; line-height:2; color:#d1d5db; font-family:inherit; }
          .article-body h2 { font-size:1.6rem; font-weight:800; color:#ffffff; margin-top:2.5rem; margin-bottom:1rem; padding-bottom:0.4rem; border-bottom:2px solid #4169e1; }
          ...
          .article-body ul li::before { content:"◈"; position:absolute; right:0; color:#4169e1; font-size:0.8rem; top:0.55rem; }
          ...
          .article-body ol li::before { content:counter(item); position:absolute; right:0; background:#4169e1; color:white; width:1.4rem; height:1.4rem; border-radius:50%; display:flex; align-items:center; justify-content:center; font-size:0.75rem; font-weight:700; top:0.2rem; }
          .article-body blockquote { border-right:4px solid #4169e1; padding:1rem 1.2rem; margin:1.5rem 0; color:#9ca3af; font-style:italic; background:#121829; border-radius:0 0.5rem 0.5rem 0; }
          .article-body a { color:#93aeff; text-decoration:none; }
          ...
          <div className="flex items-center gap-4">
            ...
            {siteInfo?.profileImageUrl ? (
              <img
                ...
                alt="مصطفى جغلال"
                className="w-14 h-14 rounded-full object-cover border-2 border-royal-500 flex-shrink-0"
              />
            ) : (
              <div className="w-14 h-14 rounded-full bg-gradient-to-br from-royal-500 to-royal-600 flex items-center justify-center font-bold text-lg flex-shrink-0">
                م
              </div>
            )}
        ...
        <div className="mt-8">
          <Link to="/articles"
            className="inline-flex items-center gap-2 text-royal-400 hover:text-blue-300 transition-colors font-semibold">
            <ArrowRight size={18} />
