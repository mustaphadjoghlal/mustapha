...
export function WorkDetailPage() {
  ...
  if (!work) return (
    <div className="min-h-screen bg-ink-950 text-white flex items-center justify-center" dir="rtl">
      <div className="text-center">
        ...
        <Link to="/portfolio-design" className="text-royal-400 hover:underline">← العودة للأعمال</Link>
      </div>
    </div>
  );
  ...
  return (
    <div className="bg-ink-950 text-white min-h-screen" dir="rtl">
      {/* Lightbox */}
      {lightboxIndex !== null && (
        <div className="fixed inset-0 bg-ink-950/95 z-50 flex items-center justify-center p-4" onClick={() => setLightboxIndex(null)}>
          ...
          <div className="relative max-w-5xl w-full" onClick={(e) => e.stopPropagation()}>
            ...
            {allImages.length > 1 && (
              <>
                <button onClick={prevImg} className="absolute right-2 top-1/2 -translate-y-1/2 bg-ink-950/60 hover:bg-ink-950/80 text-white p-3 rounded-full transition-all">
                  <ChevronRight size={24} />
                </button>
                <button onClick={nextImg} className="absolute left-2 top-1/2 -translate-y-1/2 bg-ink-950/60 hover:bg-ink-950/80 text-white p-3 rounded-full transition-all">
                  <ChevronLeft size={24} />
                </button>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pt-8">
        <Link to={backLink} className="flex items-center gap-2 text-gray-400 hover:text-royal-300 transition-colors text-sm w-fit">
          <ArrowRight size={16} />
          العودة للأعمال
        </Link>
      </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              {work.images.map((img, i) => (
                <div key={i} className="relative group aspect-square overflow-hidden rounded-xl cursor-pointer border border-ink-700 hover:border-royal-500 transition-all"
